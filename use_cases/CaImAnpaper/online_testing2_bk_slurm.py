...
if False:
    ...
    time_track_activity = []
    time_track_neurons = []
    for ind_dataset in [0,1,3,4,5,2,6,7,8,9]:
        with np.load(params_movie[ind_dataset]['folder_name']+'results_analysis_online_sensitive_SLURM_01.npz') as ld:
            ...
            names.append(params_movie[ind_dataset]['folder_name'].split('/')[4])
            num_comps.append(ld['Ab'][()].shape[-1])
            t = ld['tottime']
            pctiles.append(np.percentile(t[t<np.percentile(t,99)],[25,75])*params_movie[ind_dataset]['fr'])
            medians.append(np.percentile(t[t<np.percentile(t,99)],50)*params_movie[ind_dataset]['fr'])
            realtime.append(1/params_movie[ind_dataset]['fr'])
    ...
    pl.errorbar(num_comps,medians,yerr=pctiles.T,fmt='o')
#%%
## =============================================================================
##
## =============================================================================
